...
function encodeString(field: number, str: string): number[] {
  ...
  return [fieldTag, ...encodeVarint(text.length), ...text];
}

// 辅助：编码 varint 字段
function encodeVarintField(field: number, value: number): number[] {
  return [(field << 3), ...encodeVarint(value)];
}
...
describe('弹幕 Protobuf 解析器', () => {
  it('应能解析单条弹幕 (content + progress + color)', () => {
    ...
    expect(result.diagnostics.foundContentTags).toBe(1);
    expect(result.diagnostics.foundProgressTags).toBe(1);
  ...
  it('应处理多条弹幕', () => {
    ...
    expect(result.diagnostics.foundContentTags).toBe(2);
  });
